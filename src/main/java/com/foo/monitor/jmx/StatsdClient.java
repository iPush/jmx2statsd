...
public class StatsdClient extends TimerTask {
	...
	private boolean send(double sampleRate, String... stats) {
		...
		boolean retval = false; // didn't monitor anything
		if (sampleRate < 1.0) {
	...
	private synchronized boolean doSend(String stat) {
			...
			LOG.error(
					String.format("Could not monitor stat %s to host %s:%d", sendBuffer.toString(), _address.getHostName(),
							_address.getPort()), e);
			return false;
		}
        }
		...
		try {
			...
			if (sizeOfBuffer == nbSentBytes) {
				...
				LOG.error(String.format(
						"Could not monitor entirely stat %s to host %s:%d. Only sent %d bytes out of %d bytes", sendBuffer.toString(),
